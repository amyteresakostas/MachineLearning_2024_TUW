...
class RTA():
  ...
  def process_dataframe(self, dataset):
  ...
  def plot_heatmap(self, cramersV_results):
  ...
  def run_rta(self):  
   ...
   data = self.process_dataframe(dataset) 
   
   self.correlation_plot(data)
   ...
   for col1 in character_or_factor_columns:
       for col2 in character_or_factor_columns:
           if col1 != col2:
            cramersV_results.loc[col1, col2] = self.cramers_v(data[col1], data[col2])

   # Convert results to float for heatmap
   cramersV_results = cramersV_results.astype(float)

   self.plot_heatmap(cramersV_results)
